...
# jsonData.select(col("actor.id"),col("actor.id") > 750000).show()
# jsonData.select(col("id").alias("User_id"), col("public").alias("State"), col("actor")).show()

#SELECT WITH EXPRESSION
# jsonData.selectExpr(
#     "count(distinct(id)) as id",    #distinct: hợp nhất các bản ghi giống nhau, sau đó trả về bản ghi duy nhất không trùng lặp
#     "count(distinct(actor.id)) as actor"
# ).show()
#
# jsonData.where("actor.id  < 200000").show()

# jsonData.select(col("id"), col("actor.id").alias("ACTOR")).filter(col("id") == "2615567732").show()

from pyspark.sql.functions import length
jsonData.select(col("id")).filter(length(col("id")) < 9).show()
