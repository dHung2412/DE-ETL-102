...
# jsonData.select(col("payload.issue.state")).distinct().show()
# jsonData.select(col("payload.issue.state")).distinct().selectExpr("count(*) as count").show() # count(*) đếm cả giá trị Null

jsonData.select(col("payload.issue.state")).alias("state").dropDuplicates(["state"]).show()
