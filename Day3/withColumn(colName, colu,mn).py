...
from Day1.RDD.Transformation.TranformationOperations import flatmapRdd
...
"""
colName: tên cột
column: giá trị của cột

thêm cột mới nếu colName kh tồn tại 
ghi đè nên cột hiện có ( colName tồn tại )

col(), lit(), when(), struct() : để xác định giá trị cột 
"""

# jsonData = jsonData.withColumn("id2", lit("hungdepzai"))
# jsonData.select(col("id"), col("id2")).show()

# ADD id2 to ector structype
...
# def hungdepzai(data):
#     return data + 3
# #function UDF: user define: nguoi dung dinh nghia
# from pyspark.sql.functions import udf
# hungdepzaiUDF = udf(hungdepzai)
#
# df = jsonData.withColumn("hung09", hungdepzaiUDF(col("actor.id")))
# df.select(col("actor.id").alias("old_col"), col("hung09").alias("new_col")).show()
